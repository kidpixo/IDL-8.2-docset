...
for row in rows:
     print BeautifulSoup.BeautifulSoup(urllib.urlopen("./Documents/"+row[1])).title.string

for row in rows:
     update_query = 'UPDATE searchIndex SET name="'+BeautifulSoup.BeautifulSoup(urllib.urlopen("./Documents/"+row[1])).title.string+'" WHERE id='+str(row[0])
     print update_query
